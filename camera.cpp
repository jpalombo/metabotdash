...
CCamera* StartCamera(int width, int height, int framerate, int num_levels, bool do_argb_conversion)
{
	...
	if(GCamera != NULL)
	{
        qDebug("Can't create more than one camera");
		return NULL;
	}
...
MMAL_COMPONENT_T* CCamera::CreateCameraComponentAndSetupPorts()
{
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Unable to enable control port : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Couldn't set preview port format : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Couldn't set video port format : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Couldn't set still port format : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
...
MMAL_COMPONENT_T* CCamera::CreateSplitterComponentAndSetupPorts(MMAL_PORT_T* video_output_port)
{
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Failed to create splitter component");
		goto error;
	}
	...
	{
        qDebug("Splitter doesn't have correct ports: %d, %d",splitter->input_num,splitter->output_num);
		goto error;
	}
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Couldn't set resizer input port format : error %d", status);
		goto error;
	}

    for(int i = 0; i < (int)splitter->output_num; i++)
	{
		...
		if (status != MMAL_SUCCESS)
		{
            qDebug("Couldn't set resizer output port format : error %d", status);
			goto error;
		}
	}
...
bool CCamera::Init(int width, int height, int framerate, int num_levels, bool do_argb_conversion)
{
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Failed to enable connection");
		goto error;
	}
	...
	for(int i = 0; i < num_levels; i++)
	{
		...
		{
            qDebug("Failed to initialize output %d",i);
			goto error;
		}
	}
	...
	//return success
    qDebug("Camera successfully created");
	return true;
...
void CCamera::OnCameraControlCallback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
    Q_UNUSED(port);
    Q_UNUSED(buffer);
    qDebug("Camera control callback");
}
...
{
    qDebug("Init camera output with %d/%d",width,height);
	Width = width;
	Height = height;
	...
	if(!output_queue)
	{
        qDebug("Failed to create output queue");
		goto error;
	}
...
void CCameraOutput::OnVideoBufferCallback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
		...
		if(MMAL_BUFFER_HEADER_T* existing_buffer = mmal_queue_get(OutputQueue))
		{
			...
			if (port->is_enabled)
			{
				...
				if (!new_buffer || status != MMAL_SUCCESS)
                    qDebug("Unable to return a buffer to the video port");
			}	
		}
	}
    ...
    //qDebug("Video buffer callback, output queue len=%d", mmal_queue_length(OutputQueue));
}
...
{
    //qDebug("Attempting to read camera output");

	//try and get buffer
	if(MMAL_BUFFER_HEADER_T *buffer = mmal_queue_get(OutputQueue))
	{
        //qDebug("Reading buffer of %d bytes from output",buffer->length);
...
MMAL_COMPONENT_T* CCameraOutput::CreateResizeComponentAndSetupPorts(MMAL_PORT_T* video_output_port, bool do_argb_conversion)
{
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Failed to create reszie component");
		goto error;
	}
	...
	{
        qDebug("Resizer doesn't have correct ports");
		goto error;
	}
...
MMAL_POOL_T* CCameraOutput::EnablePortCallbackAndCreateBufferPool(MMAL_PORT_T* port, MMAL_PORT_BH_CB_T cb, int buffer_count)
{
    ...
    qDebug("Creating pool with %d buffers of size %d", port->buffer_num, port->buffer_size);
	buffer_pool = mmal_port_pool_create(port, port->buffer_num, port->buffer_size);
	if (!buffer_pool)
	{
        qDebug("Couldn't create video buffer pool");
		goto error;
	}
	...
	if (status != MMAL_SUCCESS)
	{
        qDebug("Failed to set video buffer callback");
		goto error;
	}
	...
	//send all the buffers in our pool to the video port ready for use
	{
		...
		for (q=0;q<num;q++)
		{
			...
			{
                qDebug("Unable to send a buffer to port (%d)", q);
				goto error;
			}
		}
	}
